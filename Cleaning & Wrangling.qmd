...
ggsave("slopes_spp_prez.jpg", width = 7,height = 3.5)
...
ggsave("slopes_growthxnative_prez.jpg", height = 3.5, width = 5)
...
#### Line: SPEI + V*growth
```{r}
# the model: m_spei_p_xgrowth <- glmer(total_fl_mass_ln ~ spei_avg+Veg_Mass_ln*Growth_Habit + (1|Block) + (1|Treatment) + (1|Scientific_Name), family = gaussian, data = repro_pop)

summary(repro_pop)

ggplot(repro_pop, aes(x=Veg_Mass_ln, y=total_fl_mass_ln, shape=Growth_Habit, color=spei_avg)) + geom_point() + geom_smooth(method=lm, aes(fill=Growth_Habit))


# make a facet with forb and grass with their own facet

ggplot(repro_pop, aes(x=Veg_Mass_ln, y=total_fl_mass_ln, shape=Growth_Habit, color=spei_avg)) + 
  geom_point() + geom_smooth(method=lm) + facet_grid(~Growth_Habit)

# three lines for three different SPEIs
repro_pop$Growth_Habit <- factor(repro_pop$Growth_Habit)

spei_vals <- quantile(repro_pop$spei_avg, probs = c(0.1, 0.5, 0.9), na.rm = TRUE)

pred_dat <- expand.grid(
  Veg_Mass_ln  = seq(min(repro_pop$Veg_Mass_ln, na.rm = TRUE),
                     max(repro_pop$Veg_Mass_ln, na.rm = TRUE),
                     length.out = 100),
  spei_avg     = spei_vals,
  Growth_Habit = levels(repro_pop$Growth_Habit)
)

pred_dat$pred_repro_mass <- predict(
  m_spei_p_xgrowth,
  newdata = pred_dat,
  re.form = NA,
  type = "response"
...
pred_dat <- pred_dat %>%
  mutate(
    spei_f = factor(
      spei_avg,
      labels = c("Low SPEI", "Median SPEI", "High SPEI")
    )
  )
...
ggplot(pred_dat,
  ...
  labs(
    x = "Vegetative Mass (ln)",
    y = "Predicted Reproductive Mass (ln)",
    color = "SPEI level"
  ) +
  theme_classic() +
  theme(
...
ggplot() +
  ## raw points (colored by continuous SPEI)
  geom_point(
    data = repro_pop,
    aes(
      x = Veg_Mass_ln,
  ...
  ## prediction lines (same color scale!)
  geom_line(
    data = pred_dat,
    aes(
      x = Veg_Mass_ln,
  ...
  facet_wrap(~ Growth_Habit) +
  scale_color_viridis_c(name = "SPEI") +
  labs(
    x = "Vegetative Mass (ln)",
    y = "Reproductive Mass (ln)"
  ) +
  theme_classic() +
    theme(
    legend.title = element_text(size=12,face="bold"),
...
ggsave("SPEIxgrowth_prez.jpg", height = 3.5, width = 7)
...
ggbiplot(pca, groups = pca_scores_2$Growth2, ellipse = TRUE, circle = TRUE) +
  scale_color_discrete(name = "") +
  theme_minimal()
