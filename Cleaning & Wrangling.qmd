...
comp_sum <- comp_sum %>%
  ...
  group_by(Site) %>% 
  arrange(desc(prop_adj), .by_group = TRUE)
...
# rank by prop_adj within each site -- actually just bring in final plant list and use proportions of that
...
# For now just do top 12 species, and then reader can possibly see overlap
```
...
No clear pattern here. Would need to see which species are driving these peaks and velleys.

Try it again with distance from common garden.
