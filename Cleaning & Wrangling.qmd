...
m_v_pr <- glm(total_fl_mass_ln ~ Veg_Mass_ln + Precip_mm.y, family = gaussian, data = repro_fill_hi_n_narm)

m_vxpr <- glm(total_fl_mass_ln ~ Veg_Mass_ln*Precip_mm.y, family = gaussian, data = repro_fill_hi_n_narm)

m_v_pd <- glm(total_fl_mass_ln ~ Veg_Mass_ln + precip_dif.y, family = gaussian, data = repro_fill_hi_n_narm)

m_vxpd <- glm(total_fl_mass_ln ~ Veg_Mass_ln*precip_dif.y, family = gaussian, data = repro_fill_hi_n_narm)
...
10314.2/27751 
```
Overwhelmingly the model that uses species as an explanaotry variable is best. The invasive native question would be better investigated with traits. 
